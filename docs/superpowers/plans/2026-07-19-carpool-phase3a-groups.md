...
- I/O: `fetchGroups()`, `createGroup(record)`, `fetchMyMemberships()`, `requestToJoin(groupId)`, `fetchMyRequests()`, `fetchPendingRequesters(groupId)`, `decideRequest({requestId, accept})`, `fetchRoster(groupId)`, `leaveGroup(groupId)`.
  - `requestToJoin(groupId)` calls `supabase.rpc('request_to_join', { gid: groupId })`. The RPC is also the re-request path: it clears any prior accepted/declined row for that (group, caller) pair before inserting a fresh `'pending'` one, so a declined or departed family can always ask again without a 409 on the `unique(group_id, user_id)` constraint.
  - `decideRequest({requestId, accept})` calls `supabase.rpc('accept_join_request', { request_id: requestId })` when `accept` is true, or `supabase.rpc('decline_join_request', { request_id: requestId })` when false. Each RPC is a single SECURITY DEFINER call that performs the membership insert (accept only) and the status update atomically in the database — there is no client-ordering step to get right, and a decided request can never be moved back to `'pending'` (DB-enforced, not client-enforced).
