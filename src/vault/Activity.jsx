...
import { HOUSE, WORDS } from './config.js';

export function DashboardStats({ owner, refresh }) {
  const [data,setData]=useState(null);
  const [error,setError]=useState('');
  const [retry,setRetry]=useState(0);
  useEffect(()=>{let live=true;setData(null);setError('');rewardCall('vault_my_dashboard',{p_house:HOUSE.id}).then(r=>{if(live)setData(r);}).catch(()=>{if(live)setError('Your activity totals couldn’t load.');});return()=>{live=false;};},[owner,refresh,retry]);
