...
export function BadgeIcon({ milestone, locked = false }) {
  const colors = {1:'#1a2a56',10:'#BD0032',50:'#AE622A',100:'#6F447A',250:'#287269',500:'#263E70'};
  const color = locked ? '#b7b1b3' : colors[milestone] || colors[10];
  const symbol = {
    1:<><path d="M36 19l5 10 11 2-8 8 2 11-10-5-10 5 2-11-8-8 11-2Z"/></>,
  ...
  return <svg className="merit-patch" viewBox="0 0 72 82" role="img" aria-label={`${badgeName(milestone)}: ${milestone === 1 ? 'first shared memory' : `${milestone} photos`}${locked?', not yet earned':''}`}>
