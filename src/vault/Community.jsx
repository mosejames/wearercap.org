...
export function Avatar({ owner, name, large = false, photo, hidePhoto = false }) {
  const { keys, version } = useContext(AvatarContext);
  const src = hidePhoto ? null : photo || avatarUrl(keys[owner]);
  const [broken, setBroken] = useState(false);
  useEffect(() => setBroken(false), [src, version]);
  return <span className={`avatar${large ? ' lg' : ''}`}>{src && !broken ? <img src={`${src}${src.includes('?') ? '&' : '?'}v=${version}`} alt="" onError={() => setBroken(true)} /> : (name || '?').split(/\s+/).map(w=>w[0]).join('').slice(0,2).toUpperCase()}</span>;
}
export function BadgeShelf({ owner, refresh = 0 }) {
  ...
  return <section className="badge-shelf"><h2>Your memory milestones</h2><div className="badge-list">{rewards.badges.length ? rewards.badges.map(n=><span className="earned-badge" key={n}>✦ {badgeName(n)} <small>{n} photos</small></span>) : <p>Every shared smile is a start. Your first badge is waiting at 10 photos.</p>}</div>{next && <p className="fine">{Math.max(0,next-rewards.photos)} more photos to {badgeName(next)}.</p>}</section>;
}
export function BadgeCelebration({ milestones, onClose }) {
  if(!milestones?.length) return null;
  return <aside className="badge-celebration" role="status"><button className="icon-btn" aria-label="Dismiss badge celebration" onClick={onClose}>×</button><span aria-hidden="true">✦</span><b>{badgeName(milestones.at(-1))}!</b><p>{milestones.join(', ')} photos shared. Look at all the memories you’re helping our family keep.</p><a href="#/me" onClick={onClose}>See your badges →</a></aside>;
}
